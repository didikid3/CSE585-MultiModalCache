...
def classification_pipeline(cache, image, label):
    ...
    if(res == []):
        _ = cache.get_model_output(image)
...
def plot_thresholds():
    ...
    ax1.plot(df['Threshold'], df['Cache Hit Rate'], marker='o', color='blue', linewidth=4, label="Cache Hit Rate")
    ax1.plot(df['Threshold'], df['Correct Classification Rate from Cache Hits'], marker='o', color='green', linewidth=4, label="Correct Classification Rate")

    # Right y-axis (Time Taken)
    ax2 = ax1.twinx()
    ax2.set_ylabel("Time Elapsed during Sequence (seconds)")
    ax2.set_ylim(0, max(df['Time Taken'].astype(float)) + 5)
    #print(df['Time Taken'].astype(float))

    ax2.plot(df['Threshold'], df['Time Taken'].astype(float), linestyle='--', color='gray', linewidth=2, label="Time Taken")
